...
{
	double temp = wiener->data[wiener->length-1] - K;
	if (temp > 0)
		return temp;
	else
		return 0;	
...
double approximate_Vcall(double start, double K, double my, double sigma,
	int T, double delta_t, size_t num_points, int seed)
{	
	double approximate = 0;
	array_1d_t* payoff = create_array_1d_t(num_points);
	payoff = draw_payoff_array(start, K, my, sigma, T, delta_t, num_points, seed);
	approximate = compute_mean(payoff);
	free(payoff);
	return approximate;
}

array_1d_t* draw_payoff_array(double start, double K, double my, double sigma, 
								int T, double delta_t, size_t num_points, int seed)
{
	gsl_rng* rng;
	...
	for (size_t i = 0; i < num_points; i++) {
		...
		gbm_discr = draw_gbm(start, my, sigma, delta_t, T, bm_discr);
...
/*
double Monte_Carlo_t(double(*f)(double), double start, double end, int num, int seed)
{
	gsl_rng* rng;
	rng = gsl_rng_alloc(gsl_rng_mt19937);
	array_1d_t* contain = create_array_1d_t(num);
	gsl_rng_set(rng, seed);
	double temp, result;
	for (int i = 0; i < num; i++) {
		temp = gsl_ran_flat(rng, start, end);
		contain->data[i] = f(temp);
	}
	result= compute_mean(contain);
	free_array_1d_t(contain);
	return result;
}*/